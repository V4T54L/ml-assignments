...
from easyocr.config import (
    MODULE_PATH, detection_models, recognition_models,
    all_lang_list, bengali_lang_list, arabic_lang_list,
    devanagari_lang_list, cyrillic_lang_list,
    BASE_PATH, imgH
)
...
class Reader(object):
    ...
    def __init__(self, lang_list, gpu=True, model_storage_directory=None,
        ...
        self.corrupt_msg = 'MD5 hash mismatch, possible file corruption'
        if detector:
        ...
        if recog_network in ['standard'] + [model for model in recognition_models['gen1']] + [model for model in recognition_models['gen2']]:
            ...
            if recognizer:
                if os.path.isfile(model_path) == False:
                    ...
                    assert calculate_md5(model_path) == model['md5sum'], self.corrupt_msg
                    ...
                    LOGGER.warning(self.corrupt_msg)
    ...
    def getDetectorPath(self, detect_network):
        if detect_network in self.support_detection_network:
            ...
            if os.path.isfile(detector_path) == False:
                ...
                assert calculate_md5(detector_path) == self.detection_models[self.detect_network]['md5sum'], self.corrupt_msg
                LOGGER.info('Download complete')
